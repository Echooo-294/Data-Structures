...
void Prim1(Graph* G, int* D, int s) { // Prim's MST algorithm
...
public:
  int vertex, distance;
  DijkElem_() { vertex = -1; distance = -1; }
  DijkElem_(int v, int d) { vertex = v; distance = d; }
};

// Comparator class for comparing DijkElem's
class DDComp_ {
public:
  static bool prior(DijkElem_ x, DijkElem_ y)
...
void Prim2(Graph* G, int* D, int s) {
  ...
  DijkElem_ temp;
  DijkElem_ E[G->e()];    // Heap array with lots of space
  temp.distance = 0; temp.vertex = s;
  E[0] = temp;           // Initialize heap array
  heap<DijkElem_, DDComp_> H(E, 1, G->e()); // Create heap
...
//       q.pop();
//     }
//     while(G->getMark(v) == VISITED) ; // 这个queue里可能有重复的节点。因为下面那个更新的没有管q里先前有没有相同的。
...
//     if (D[v] == INFINITY) return;      // 这图已经是最大生成子图了
